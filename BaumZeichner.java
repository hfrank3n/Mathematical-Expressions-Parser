...
public class BaumZeichner extends Frame {
    ...
    public BaumZeichner(int frameWidth, int frameHeight, BinaryTree<String> pBaum) {
        super("BinaryTree");
        _baum = pBaum;
    ...
    private void zeichnePreorder(int links, int rechts, int y, BinaryTree baum)
    {  if (baum.isEmpty()) return;
        int mitte = (links + rechts) / 2;
        zeigeKnoten(links, rechts, y, baum.getContent().toString());
        zeichnePreorder(links, mitte,y+_dy,baum.getLeftTree());
        zeichnePreorder(mitte, rechts,y+_dy,baum.getRightTree());
    }
